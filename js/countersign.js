/* ---------- Authorised-person countersigning shell (countersign.html only) ----------
   Mirrors js/sign.js's participant flow, but fetches the participant-signed
   PDF via the authorised token, only exposes the "arete" signature field, and
   posts the final flattened PDF back to flip status to FullyExecuted. */
